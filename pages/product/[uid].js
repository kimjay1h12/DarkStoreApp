...
function Index() {
  ...
  const FetchProductDetails = async () => {
    setLoading(true);
    try {
      const res = (await client.get(`/products/${uid}`))
        .data;
          ...
          <Grid container spacing={6}>
            <Grid item xs={12} sm={12} md={6}>
              ...
              <Carousel
                ...
                showThumbs={false}
              >
                {productDetails?.image?.map((cur, i) => (
                  <img src={cur} className={classes.img} key={i} />
                ))}
              </Carousel>
            </Grid>
            <Grid item xs={12} sm={12} md={6}>
              <div>
                ...
                {productDetails?.color?.length>0&& <Typography fontFamily="revert-layer" gutterBottom mt={3}>
                  Avaliable Colors
                </Typography>}
                <ButtonGroup variant="contained" color="primary" aria-label="color selector">
              
        {productDetails?.color?.length>0&& productDetails?.color?.map((cur) => (
          <Button
            key={cur}
            onClick={() => {
         if(cur === color){
          setColor("")
           ...
           setColor(cur)
            ...
            style={{
              backgroundColor: cur.toLowerCase(),
              // color: 'white',
              boxShadow:"none",
              
              opacity: color ===cur ? 10 : 0.7,
            }}
          >
          {color === cur?<CheckIcon/>:  cur}
          </Button>
        ))}
      </ButtonGroup>
               {productDetails?.size?.length>0&& <Typography fontFamily="revert-layer" gutterBottom mt={3}>
                 Avaliable Sizes
                </Typography>}
